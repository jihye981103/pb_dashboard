...
def get_image_map():
    ...
    try:
        ...
        print(f"\n[디버깅] 구글 드라이브에서 발견한 파일 개수: {len(files)}개")
        for file in files:
            file_name = file.get('name')
            file_id = file.get('id')
            p_code = os.path.splitext(file_name)[0].strip()
            image_map[p_code] = file_id
            print(f"  - 드라이브 파일명: '{file_name}' -> 추출된 품목코드: '{p_code}'")
...
def create_pdf(selected_data, image_map, items_per_page):
    ...
    for category, group in selected_data.groupby('카테고리', sort=False):
        ...
        for _, row in group.iterrows():
            ...
            if image_id:
                print(f"  [성공] 시트 품목코드 '{p_code}' ({p_name}) -> 이미지 매칭됨!")
                if service:
                    try:
                        ...
                        print(f"  [오류] 이미지 다운로드 에러: {e}")
            else:
                print(f"  [실패] 시트 품목코드 '{p_code}' ({p_name}) -> 드라이브에서 찾지 못함")
